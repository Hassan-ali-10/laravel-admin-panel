...
@section('content')
<div class="container">
    <h1>{{ isset($sale) ? 'Edit Sale' : 'Add Sale' }}</h1>
    <form action="{{ isset($sale) ? route('admin.sales.update', $sale) : route('admin.sales.store') }}" method="POST">
        @csrf
        @if(isset($sale))
        @method('PUT')
        @endif
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="date" name="date" value="{{ isset($sale) ? $sale->date : '' }}" required>
        </div>
